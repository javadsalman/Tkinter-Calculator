...
class Process():
    ...
    def _arithmetic(self, expression):
        if self.protext:
            if self.protext[-1].isdigit():
                ...
                self._show()
        elif not self.protext and expression == '-':
                self._add('0-')
        else:
    ...
    def mequal(self):
        ...
        problem_string = self.protext.translate({
            ord('+'): ' + ',
            ord('-'): ' - ',
            ord('*'): ' * ',
            ord('/'): ' / ',
        })
        calc_result = ca.evaluate(problem_string)
        self.protext = str(int(calc_result)) if calc_result.is_integer() else str(round(calc_result, 7))
        self._show()
